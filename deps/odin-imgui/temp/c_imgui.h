...
CIMGUI_API bool ImGui_BeginPopupContextWindow(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);  // open+begin popup when clicked on current window.
CIMGUI_API bool ImGui_BeginPopupContextVoid(const char* str_id /* = NULL */, ImGuiPopupFlags popup_flags /* = 1 */);    // open+begin popup when clicked in void (where there are no windows).
...
CIMGUI_API void ImGui_TableNextRow(ImGuiTableRowFlags row_flags /* = 0 */, float min_row_height /* = 0.0f */); // append into the first cell of a new row.
...
CIMGUI_API bool        ImGui_IsKeyPressed(ImGuiKey key, bool repeat /* = true */);               // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
...
CIMGUI_API bool   ImGui_BeginChildFrame(ImGuiID id, ImVec2 size, ImGuiWindowFlags window_flags /* = 0 */);
