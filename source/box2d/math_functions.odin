...
// from the origin. In single precision (the default build) this is identical to Vec2.
Pos :: Vec2

// A world transform with double precision translation and float rotation. Rotation is frame
// local and never needs the extra range, the same split as Jolt's DMat44. Identical to
// Transform in single precision.
...
@(require_results)
RoundDownFloat :: proc "c" (x: f32) -> f32 {
	return x
}
...
@(require_results)
SubPos :: proc "c" (a, b: Pos) -> Vec2 {
	return {a.x - b.x, a.y - b.y}
}

// p + d
@(require_results)
OffsetPos :: proc "c" (p: Pos, d: Vec2) -> Pos {
...
@(require_results)
TransformWorldPoint :: proc "c" (t: WorldTransform, p: Vec2) -> Pos {
	rx := t.q.c * p.x - t.q.s * p.y
	ry := t.q.s * p.x + t.q.c * p.y
	return {t.p.x + rx, t.p.y + ry}
}

// Transform a world position to a local point.
@(require_results)
InvTransformWorldPoint :: proc "c" (t: WorldTransform, p: Pos) -> Vec2 {
	vx := p.x - t.p.x
	vy := p.y - t.p.y
	return {t.q.c * vx + t.q.s * vy, -t.q.s * vx + t.q.c * vy}
}
...
InvMulWorldTransforms :: proc "c" (A, B: WorldTransform) -> (C: Transform) {
	C.q = InvMulRot(A.q, B.q)
	d := Vec2{B.p.x - A.p.x, B.p.y - A.p.y}
...
@(require_results)
OffsetWorldTransform :: proc "c" (A: WorldTransform, B: Transform) -> (C: WorldTransform) {
	C.q = MulRot(A.q, B.q)
	C.p = OffsetPos(A.p, RotateVector(A.q, B.p))
	return
}
...
@(require_results)
IsValidPosition :: proc "c" (p: Pos) -> bool {
	return IsValidVec2(p)
}

// Is this a valid transform? Not NaN, and rotation is normalized.
@(require_results)
IsValidTransform :: proc "c" (t: Transform) -> bool {
	IsValidVec2(t.p) or_return
	return IsValidRotation(t.q)
}

// Is this a valid world transform? Not NaN, and rotation is normalized.
@(require_results)
